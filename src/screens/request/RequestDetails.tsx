...
export default function RequestDetails(props: any) {
  ...
  const isImageDocument = (url: string) => {
    if (!url) return false;
    return /\.(png|jpe?g|gif|webp|bmp|heic|heif|svg)(\?.*)?$/i.test(url);
  };
  ...
  async function getReviewData(serviceId: any) {
    ...
    try {
      ...
      const result: any = await API.Instance.get(
        API.API_ROUTES.getReviewData + `/${serviceId}?type=both`,
      );
      if (result.status) {
  ...
  async function getAvailableCoupons() {
    try {
      setLoading(true);
      const result: any = await API.Instance.get(
        API.API_ROUTES.getAvailableCoupons,
      );
      if (result.status) {
        console.log('result==>', result?.data);
  ...
  async function onValidateCoupon(couponCode: any) {
    if (!couponCode) {
      ...
      try {
        setLoading(true);
        const result: any = await API.Instance.get(
          API.API_ROUTES.onValidateCoupon + `?coupon_code=${couponCode}`,
        );
        if (result.status) {
          console.log('result==>', result?.data);
  ...
  return (
    <View style={[styles(theme).container]}>
        ...
        {status === 'pending' && (
          <>
            ...
            <FlatList
              ...
              renderItem={({ item, index }) => {
                const shouldShowImagePreview = isImageDocument(item);
                if (shouldShowImagePreview) {
                  return (
                    <TouchableOpacity
                      ...
                      <Image
                        style={styles(theme).photosVieDocumant}
                        source={{ uri: item }}
                      />
                    </TouchableOpacity>
                  );
                  ...
                  return (
                    <TouchableOpacity
                      style={[styles(theme).uploadButton, { borderStyle: 'dashed', borderWidth: 1 }]}
                      activeOpacity={1}
                      onPress={() => {
                        props.navigation.navigate(
                          SCREENS.WebViewScreen.identifier,
      ...
      {status === 'pending' && (
        <AcceptBottomPopup
          onRef={acceptRef}
          title={t('confirm_service_with_rate_and_provider', {
            amount: serviceDetails?.total_renegotiated ?? '',
            providerName: serviceDetails?.provider?.full_name ?? '',
          })}
          onClose={() => {
            console.log(
              'serviceDetails?.total_renegotiated?.[0]',
